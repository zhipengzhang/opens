...
void AggDBWriter::createUclNumPerLayerTable()
{
	...
	if ( partitionNum == "" )
		...
		numPerLayertbName = string("agg")+"_"+partitionNum+"_"+string("numperlayer");
...
void AggDBWriter::createHdrTable()
{
	...
	if ( partitionNum == "" )
		...
		hdrtbName = string("agg")+"_"+partitionNum+"_"+string("hdr");
...
void AggDBWriter::createMapTable()
{
	...
	for ( int i = 0; i < dimsNum; i++ )
	{
		...
		if ( partitionNum == "" )
			...
			mapTableName = string("agg")+"_"+partitionNum+"_"+string("map")+itos(i+1);
...
void AggDBWriter::createUclTable()
{
	...
	for ( i = 0; i <= dimsNum; ++i )
	{
		...
		if ( partitionNum == "" )
			...
			uclTableName = string("agg")+"_"+partitionNum+"_"+string("ucl")+itos(i);
...
void AggDBWriter::createTable( string tableName, vector<string> attrName, vector<string> attrType )
{
	...
	//for(int i = 0;i<pDB->m_listRecordsets.size();i++)
	//{
	//	(pDB->m_listRecordsets)[i]->

	//}
	
	string createTableSQL = "create table \"SYSTEM\".\"" + tableName + "\" (\n";   //by hch
...
void AggDBWriter::insertHdrData( vector<string>& hdrData )
{
	...
	string insertHdrSQL = "";
	insertHdrSQL = string("insert into \"SYSTEM\".\"") + hdrtbName + "\" values ( ";  //by hch
...
void AggDBWriter::insertMapData( string& mapStr, int dim )
{
	...
	{
		string insertMapSQL = string("insert into \"SYSTEM\".\"") + maptbNames[dim] + "\" values ('"+ mapData[i] + "'," + itos(i+1) + ");";  //by hch
		//string insertMapSQL = string("insert into ") + maptbNames[dim] + " values ('"+ mapData[i] + "'," + itos(i+1) + ");";
		try
		{
			//cout<<insertMapSQL<<endl;
...
void AggDBWriter::insertUclData( int* dimData, double* msrData, int layer )
{
	...
	string insertUclSQL = string("insert into \"SYSTEM\".\"") + ucltbNames[layer] + "\" values ( ";   //by hch
...
void AggDBWriter::insertNumPerLayer( vector<int>& aggNumPerLayer )
{
	...
	string insertNumPerLayerSQL = string("insert into \"SYSTEM\".\"") + numPerLayertbName + "\" values ( ";  //by hch
