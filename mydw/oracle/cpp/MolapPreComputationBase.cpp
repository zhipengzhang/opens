...
void MolapPreComputationBase::setAttribute()
{
	...
	{ 
		...
		if ( !InXDB.IsOpen() )
		{
			...
			cout << URL << " " << username << " " << password << endl;  //by hch2
			
			/*
			string URL = "DW_wz";
			string username = "system";
			string password = "manager";   //by hch2
			*/
			try
			{
				connectNum++;
...
string MolapPreComputationBase::buildSQL()
{
	...
	for( int i=0; i < dimensions.size(); i++ )
	{
		...
		{	
			...
			formSQL+="\"SYSTEM\".\"" + dimTableToInsert+"\",";       //by hch
		...
		{
			string temp = "\"" + hierarchies[ii]->getFactTable()+"\".\""+
				hierarchies[ii]->getPrimaryKey()+"\"=\""+this->cube->getFactTable()+"\".\""+
				dimensions[i]->getForeignKey() + "\"";   //by hch
			/*
			...
			for (int j=0;j<levels.size();j++)
			{
				...
				{			 
					...
					{
						string t= "\"SYSTEM\".\"" + hierarchies[ii]->getFactTable()+"\".\""+levels[j]->getColumn() + "\""; //by hch
	...
	formSQL+="\"SYSTEM\".\"" + facttablename + "\"";    //by hch
	//formSQL+=facttablename;
	for (i=0;i<measures.size();i++)
	{
		selectSQL += "\"SYSTEM\".\"" + this->cube->getFactTable()+"\".\""+measures[i]->getColumn()+"\","; //by hch
...
void MolapPreComputationBase::loadDataTable( int start, int end )
{
	...
	try{
		...
		if ( pRS == NULL )
		{
			...
			string countSQL="select count(*) from \"SYSTEM\".\""+cube->getFactTable() + "\"";   //by hch
			...
			int recordnumber = stoi(countstring);
			conutXRS.Close();
		...
		//cout << maxRecordNum << " " << dimsNum << " " << msrsNum<<endl;   //by hch22
