...
void testSQLType()
{
	...
	string strSQLType = XODBCDataSource::GetSQLType(DB_SQLSERVER,SQL_VARCHAR);
	cout<<strSQLType.c_str()<<endl;
	strSQLType = XODBCDataSource::GetSQLType(DB_SQLSERVER,SQL_LONGVARCHAR);
	cout<<strSQLType.c_str()<<endl;
	strSQLType = XODBCDataSource::GetSQLType(DB_SQLSERVER,SQL_WVARCHAR);
	cout<<strSQLType.c_str()<<endl;
	strSQLType = XODBCDataSource::GetSQLType(DB_SQLSERVER,SQL_LONGVARCHAR);
	cout<<strSQLType.c_str()<<endl;
	strSQLType = XODBCDataSource::GetSQLType(DB_SQLSERVER,SQL_LONGVARCHAR);
...
void testDBType()
{
	...
	string strDSName;
	strDSName = "NorthWind";
	XDBTYPE tp = XODBCDataSource::GetDBType(BOTH_DSN,strDSName);
	switch(tp)
	{
	case DB_SQLSERVER:
		cout<<"A database of SQL Server"<<endl;
		break;
	case DB_MYSQL:
		cout<<"A database of MySQL"<<endl;
		break;
	case DB_UNKNOWN:
		cout<<"Unknown Database\n";
		break;
	case DB_ERROR:
		cout<<"Cannnot find the DSN!"<<endl;
		break;
	default:
		break;
	}
